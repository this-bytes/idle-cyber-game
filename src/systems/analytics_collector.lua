-- AnalyticsCollector - Privacy-respecting game analytics (systems path)
...
function AnalyticsCollector:initialize()
    if self.eventBus then
        self.eventBus:subscribe("contract_completed", function(data) self:trackEvent("contract_completed", data) end)
        self.eventBus:subscribe("specialist_hired", function(data) self:trackEvent("specialist_hired", data) end)
        self.eventBus:subscribe("upgrade_purchased", function(data) self:trackEvent("upgrade_purchased", data) end)
        self.eventBus:subscribe("threat_mitigated", function(data) self:trackEvent("threat_mitigated", data) end)
        self.eventBus:subscribe("Incident_resolved", function(data) self:trackEvent("Incident_resolved", data) end)
        self.eventBus:subscribe("synergy_activated", function(data) self:trackEvent("synergy_activated", data) end)
    end
    return true
end

function AnalyticsCollector:trackEvent(eventType, data)
    local event = { type = eventType, timestamp = os.time(), data = data }
    ...
    if eventType == "contract_completed" then
        ...
        if not self.session.first_contract then self.session.first_contract = os.time() end
        if data and data.reward then self.session.money_earned = self.session.money_earned + data.reward end
    elseif eventType == "specialist_hired" then
        self.session.specialists_hired = self.session.specialists_hired + 1
        if not self.session.first_specialist then self.session.first_specialist = os.time() end
    elseif eventType == "upgrade_purchased" then
        self.session.upgrades_purchased = self.session.upgrades_purchased + 1
        if not self.session.first_upgrade then self.session.first_upgrade = os.time() end
    ...
    elseif eventType == "Incident_resolved" then
        ...
        if not self.session.first_Incident then self.session.first_Incident = os.time() end
    end
    if #self.session.events > 1000 then table.remove(self.session.events, 1) end
end
...
function AnalyticsCollector:getStats()
    ...
    return {
        session = { duration = sessionDuration, playtime = self.session.total_playtime, events_recorded = #self.session.events },
        activity = { contracts_completed = self.session.contracts_completed, specialists_hired = self.session.specialists_hired, upgrades_purchased = self.session.upgrades_purchased, threats_mitigated = self.session.threats_mitigated, crises_resolved = self.session.crises_resolved, money_earned = self.session.money_earned },
        milestones = { first_contract = self.session.first_contract, first_specialist = self.session.first_specialist, first_upgrade = self.session.first_upgrade, first_Incident = self.session.first_Incident },
        rates = {
            contracts_per_minute = sessionDuration > 0 and (self.session.contracts_completed / (sessionDuration / 60)) or 0,
            money_per_minute = sessionDuration > 0 and (self.session.money_earned / (sessionDuration / 60)) or 0
        }
    }
end
...
function AnalyticsCollector:getRecentEvents(count)
    ...
    local recent = {}
    for i = start, #self.session.events do table.insert(recent, self.session.events[i]) end
    return recent
end
