...
function ProcGen.new(itemRegistry, formulaEngine)
    ...
    self.nameParts = {
        adjectives = {"Agile","Secure","Digital","Cloud","Quantum","Cyber","Smart","Rapid","Global","Dynamic","Elite","Prime","Advanced","Innovative","Strategic","Tactical","Unified"},
        techNouns = {"Systems","Solutions","Technologies","Networks","Security","Data","Analytics","Platforms","Services","Labs","Infrastructure","Operations","Defense","Intelligence"},
        companySuffixes = {"Inc","Corp","LLC","Ltd","Group","Partners","Ventures","Holdings","Enterprises","Consulting"},
        founderNames = {"Alice","Bob","Carol","Dave","Eve","Frank","Grace","Henry","Iris","Jack","Kate","Leo","Maya","Nathan"}
    }
...
function ProcGen:generateContract(template, playerContext)
    ...
    if template.variations then
        if template.variations.client_name then generated.clientName = self:generateName(template.variations.client_name) end
        if template.variations.risk_multiplier then
            local riskMult = self:sampleDistribution(template.variations.risk_multiplier)
            generated.riskMultiplier = riskMult
            if generated.riskLevel then
                if riskMult > 1.5 then generated.riskLevel = "EXTREME"
                elseif riskMult > 1.2 then generated.riskLevel = "HIGH"
                elseif riskMult < 0.8 then generated.riskLevel = "LOW" end
            end
        end
        if template.variations.budget_scaling and generated.baseBudget then
            local scaledBudget = self.formulaEngine.evaluate(template.variations.budget_scaling.formula or "base * risk_multiplier", { base = generated.baseBudget, player_level = playerContext.level or 1, risk_multiplier = generated.riskMultiplier or 1.0 })
...
function ProcGen:rollWeightedTable(tbl)
    local totalWeight = 0
    for _, entry in ipairs(tbl) do totalWeight = totalWeight + (entry.weight or 1) end
    local roll = math.random() * totalWeight
    local acc = 0
    for _, entry in ipairs(tbl) do acc = acc + (entry.weight or 1); if roll <= acc then return entry.value end end
    return nil
end
...
function ProcGen:deepCopy(original)
    if type(original) == "table" then
        local copy = {}
        for k, v in pairs(original) do copy[k] = self:deepCopy(v) end
        return copy
    else
        return original
    end
